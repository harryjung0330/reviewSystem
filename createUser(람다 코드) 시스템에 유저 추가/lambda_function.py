...
def lambda_handler(event, context):
    ...
    #print(form_data)
    #name = filterData(form_data["name"][0])
    ...
    name = form_data["name"]
    phoneNumb = filterData(form_data["phoneNumb"])
    socialSec = filterData(form_data["socialSec"])
    logInId = filterData(form_data["logInId"])
    psw = filterData(form_data["psw"])
    country = filterData(form_data["country"])
    city = filterData(form_data["city"])
    streetAddr = filterData(form_data["streetAddr"]) 
    detailAddr = filterData(form_data["detailAddr"])
    emailAddr = filterData(form_data["emailAddr"])
    ...
    try:
        ...
        cursor = connection.cursor()
        userId = createUser(cursor, name, phoneNumb, socialSec, logInId, psw, country, city , detailAddr , emailAddr)
        resPondMsg = "Success!"
    ...
    return {
        ...
        'body': json.dumps({"respondMsg": resPondMsg, "resFlag": resFlag})
    }
