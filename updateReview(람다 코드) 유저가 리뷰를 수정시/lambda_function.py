...
def lambda_handler(event, context):
    ...
    try:
        ...
        cursor.execute("""SELECT userSysId FROM review WHERE reviewId = {}""".format(reviewId))
        reviewWriter = cursor.fetchone()[0]
        print("reviewWriter is " + str(reviewWriter))
        if(userSysId != reviewWriter):
            return {'statusCode': 200,
            'body': json.dumps({
                "respondMsg": "Wrong person to update!",
                "resFlag": 1
            })}
        ...
        for i in range(0, pictNum):
            pictList.append("{}/{}.jpg".format(reviewId, i))
            
        updateReview(cursor, reviewId, reviewContent, rating, prodList = prodList,pictList = pictList)
        resPondMsg = "Success!"
